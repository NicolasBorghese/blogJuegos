...
<div class="bg-white rounded-lg shadow-md overflow-hidden w-1/2 mx-auto p-6">
    <h1 class="text-2xl font-bold mb-4 text-slate-900">CREAR CUENTA</h1>

    <form action="{{ route('cuenta.store') }}" method="POST" class="space-y-4">
        @csrf

        <div>
            <label class="block text-gray-700">Nombre:</label>
            <input type="text" name="name" value="{{ old('name') }}" class="mt-1 block w-full px-3 py-2 bg-white border border-gray-500 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            @error('name')
